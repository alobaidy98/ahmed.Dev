...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == 'mute_user' then
		...
		if is_muted_user(chat_id, user_id) then
			...
			send_large_msg(receiver, "["..user_id.."] 😉 جراره ❤️  راح النلصم منك يلا دردش 💔")
		elseif is_admin1(msg) then
			mute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] ✋🏻عميت على نفسك تم نلصمك 🤐")
		end
	end
end
...
local function run(msg, matches)
	if msg.to.type == 'chat' then
		if matches[1] == 'ترقية السوبر' then
	...
	local name_log = print_name:gsub("_", " ")
	local data = load_data(_config.moderation.data)
		if matches[1] == 'تفعيل المجموعة' and not matches[2] then
		...
		if matches[1] == 'تعطيل المجموعة' and is_admin1(msg) and not matches[2] then
		...
		if matches[1] == "مدير المجموعة" then
		...
		if matches[1] == 'غير رابط' and is_momod(msg)then
			local function callback_link (extra , success, result)
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, '*✋🏻❌ عذرا لا يمكن تغير رابط هاذه المجموعه 👍* \nالمجموعه ليست من صنع البوت.\n\nيرجئ استخدام رابط الخاص بها في اعدادات المجموعه')
					data[tostring(msg.to.id)]['settings']['set_link'] = nil
					save_data(_config.moderation.data, data)
				else
					send_large_msg(receiver, "تم✔️ غير رابط 👥 ")
		...
		if matches[1] == 'رابط' then
			...
			if not group_link then
				return "❓يرجئ ارسال [/غير رابط] لانشاء رابط المجموعه👍🏻✔️"
			end
		...
		--[[if matches[1] == 'kick' and is_momod(msg) then
			...
			elseif matches[1] == 'صعد اداري' and string.match(matches[2], '^%d+$') then
			--[[]	local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'setadmin'
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})]]
				local	get_cmd = 'صعد اداري'
				local	msg = msg
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'صعد اداري' and not string.match(matches[2], '^%d+$') then
				--[[local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'setadmin'
				}
		...
		if matches[1] == 'صعد مدير' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'صعد مدير',
					msg = msg
			...
			elseif matches[1] == 'صعد مدير' and string.match(matches[2], '^%d+$') then
		--[[	local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					local receiver = get_receiver(msg)
				...
				local	get_cmd = 'صعد مدير'
				local	msg = msg
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'صعد مدير' and not string.match(matches[2], '^%d+$') then
				local	get_cmd = 'صعد مدير'
				local	msg = msg
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'صعد ادمن' then
		  if not is_momod(msg) then
			...
			elseif matches[1] == 'صعد ادمن' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'صعد ادمن'
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] promoted user#id"..matches[2])
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'صعد ادمن' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'صعد ادمن',
				}
		...
		if matches[1] == "ضع المعرف" and is_admin1(msg) then
			local function ok_username_cb (extra, success, result)
				local receiver = extra.receiver
				if success == 1 then
					send_large_msg(receiver, "تم ✔️ وضع المعرف للمجموعه 👥⁉️\n\n✋🏻 انضر الئ الحول لتشاهد تغيرات المجموعه 👥")
		...
		if matches[1] == 'ضع الصورة' and is_momod(msg) then
		...
		if matches[1] == 'مسح' then
			...
			if matches[2] == 'الادمنية' then
				...
				return '❌ تم مسح قائمه الادمنية ✔️'
			end
			...
			if matches[2] == 'الملصومين' then
				chat_id = msg.to.id
				local hash =  'mute_user:'..chat_id
					redis:del(hash)
				return "❌ تم مسح قائمه الملصومين ✔️"
			end
		...
		if matches[1] == 'ضع التكرار' then
			...
			if tonumber(matches[2]) < 5 or tonumber(matches[2]) > 20 then
				return "😈 ضع التكرار من 5 الئ 20 ✔️"
			end
		...
		if matches[1] == "الملصومين" and is_momod(msg) then
			local chat_id = msg.to.id
